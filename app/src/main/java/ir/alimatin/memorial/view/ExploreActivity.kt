...
import ir.alimatin.memorial.databinding.ActivityExploreBinding
...
class ExploreActivity : AppCompatActivity() {
    ...
    private lateinit var viewDataBinding: ActivityExploreBinding
    lateinit var sharedPrefs: SharedPrefs

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        viewDataBinding = DataBindingUtil.setContentView(this, R.layout.activity_explore)

        contInst = this
        sharedPrefs = SharedPrefs(contInst)
        postViewModel = ViewModelProvider(this).get(PostViewModel::class.java)

        postViewModel.getPosts().observe(this, Observer { data ->
            initExplore(data)
        })
