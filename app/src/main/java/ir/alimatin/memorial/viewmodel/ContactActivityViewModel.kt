...
class ContactActivityViewModel(val mApplication: Application) : AndroidViewModel(mApplication) {

    private val _contactsLiveData = MutableLiveData<ArrayList<DataContact>>()

    fun fetchContacts(): LiveData<ArrayList<DataContact>> {
...
//            }
    ...
    private suspend fun getPhoneContacts(): ArrayList<DataContact> {
        val contactsList = ArrayList<DataContact>()
            ...
            while (contactsCursor.moveToNext()) {
                ...
                if (name != null) {
                    if (contactsCursor.getInt(contactsCursor.getColumnIndex(ContactsContract.Contacts.HAS_PHONE_NUMBER)) > 0) {
                        val pCur: Cursor? = mApplication.contentResolver?.query(
                        ...
                        contactsList.add(DataContact(id = id.toInt(), name = name, number = phoneNo))
                        pCur?.close()
                    }
                }
            }
